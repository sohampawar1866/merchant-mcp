...
func (a *App) WriteEnvAndLaunch(keyID, keySecret, webhookSecret, transport, port string) LaunchResult {
	...
	envContent := fmt.Sprintf(`# AgenticCheckout — Auto-generated by installer on %s
RAZORPAY_KEY_ID=%s
RAZORPAY_KEY_SECRET=%s
RAZORPAY_WEBHOOK_SECRET=%s
MCP_TRANSPORT=%s
PORT=%s
DATABASE_URL=postgres://agentic:agentic@localhost:5432/agentic_checkout?sslmode=disable
REDIS_URL=redis://localhost:6379
ENABLE_FIND_AND_PRICE=true
ENABLE_NEGOTIATION=true
ENABLE_HUMAN_APPROVAL=false
MAX_NEGOTIATION_ATTEMPTS=3
ENABLE_CATALOG_CACHE=true
...
`,
	...
	if err := os.WriteFile(envPath, []byte(envContent), 0600); err != nil {
		return LaunchResult{Success: false, Error: fmt.Sprintf("Failed to write .env file: %v", err)}
	}

	wailsRuntime.EventsEmit(a.ctx, "log", fmt.Sprintf("✓ Credentials saved to %s", envPath))
